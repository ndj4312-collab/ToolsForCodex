...
14. Route accepted architecture/spec/ticket state requiring verified realization to `/traverse`. Traverse freezes the current target and completion denominator, executes only the blocker-satisfied frontier through existing `/implement`/`/tdd`/`/code-review` owners, diagnoses failures to the earliest responsible upstream stage, replays only affected downstream closure, and requires regression plus independent quality convergence before handing accepted recurring behavior to `/endure`. Do not use Traverse to redesign upstream decisions or duplicate implementation methodology.
