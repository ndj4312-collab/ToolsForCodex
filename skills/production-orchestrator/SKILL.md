...
10. Invoke `/evidence-opinion-router` **before** `skill-context-builder` for every multi-type, compound, or cross-surface command. It owns initial task/surface classification and any evidence-backed opinion loop; purely deterministic compound work should be classified and forwarded without paying the research cost.
11. Invoke `skill-context-builder` after that pre-routing step whenever an orchestration, workflow, skill build, multi-agent task, or complex execution needs a canonical current-state context model, surface/dependency map, worker-kernel contracts, or persistent context reconciliation. For single-type work that does not meet the evidence-opinion-router trigger, it may still be invoked directly.
12. Invoke `/agentic-intel` when external intelligence must be researched and converted into agentic-system implications, primitives, current-surface deltas, converged proposals, or realization work. Preserve narrower exact owners and do not route generic summaries/news/non-agentic research through Agentic Intel. When an agentic research branch is discovered by `/evidence-opinion-router`, let `/agentic-intel` own that atomic branch and return evidence into the router's graph.
