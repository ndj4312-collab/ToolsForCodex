...
for(const r of records){
  ...
  if(r.path.startsWith('approved-proposals/')){items.push({id:r.id,path:r.path,disposition:'PRESERVE_PROPOSAL',reason:`Approved-proposal artifact ${r.path} is preserved but is not an active runtime surface until promoted and accepted.`,futureTrigger:`Re-evaluate when ${r.path} is selected for promotion or a promoted descendant changes.`,route:{type:'EXPLICIT_BLOCKER',target:null,status:'BLOCKED',blocker:'Approved proposal requires promotion and acceptance before ACTIVE routing.'}});continue;}
