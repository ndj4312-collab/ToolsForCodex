...
- For `/ingest-and-endure`, repository/skill-library re-assimilation, or any request to ingest a corpus of tools, skills, playbooks, runtimes, methodologies, standards, evaluators, or references, route through `skills/ingest-and-endure/SKILL.md` and the executable coverage gates in `src/ingestion/coverage.ts`. Representative sampling, denominator narrowing, mass PRESERVE/OBSERVE/REVISIT, unread COMPLETE states, and ACTIVE routes without acceptance evidence are forbidden.
- Approved proposal capabilities promoted by the exhaustive ingestion lifecycle MUST resolve through `pointer-catalog/ingestion/APPROVED_PROPOSAL_LIFECYCLE_REGISTRY.json` and `pointer-catalog/ingestion/APPROVED_PROPOSAL_ACTIVE_ROUTES.json`. No approved proposal may become ACTIVE unless its per-item SPEC, TICKET, IMPLEMENTATION, ACCEPTANCE, ENDURE, and WIKI records are all terminal; CI enforces exactly 456/456 for the current approved-proposal corpus.
